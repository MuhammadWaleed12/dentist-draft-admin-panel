...
import { getPracticeStatus } from "@/lib/practice-hours";
import {
  getAvailableDates,
...
export function BookingPage({ practiceId }: BookingPageProps) {
  ...
  // Generate dynamic dates and times
  useEffect(() => {
    if (practice) {
      // Generate available dates starting from current display date
      const dates = getAvailableDates(practice, 42, currentDisplayDate);
      setAvailableDates(dates);

      // Update current month display
      const monthDisplay = getCurrentMonthDisplay(practice, currentDisplayDate);
      setCurrentMonth(monthDisplay);

      // Generate time slots for selected date
      if (selectedDateObj) {
        const slots = getAvailableTimeSlots(practice, selectedDateObj);
        setTimeSlots(slots);
      } else {
        setTimeSlots([]);
      }
    }
  }, [practice, selectedDateObj, currentDisplayDate]);
  ...
  const handleDateSelect = (dateInfo: AvailableDate) => {
    ...
    setSelectedDateObj(dateInfo.fullDate);
  ...
  const handleContinue = () => {
    if (!selectedService || !selectedDateObj || !selectedTime || !practice) {
  ...
  const specialties: string[] = practice.tags || [];
  const hours = { today: "Open: 9:00 AM - 5:00 PM" }; // Default hours
  const services: Service[] = (practice.tags || []).map((tag: string) => ({
    name: tag,
  ...
  // Get practice status for badge
  const practiceStatus = practice ? getPracticeStatus(practice) : null;
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="relative aspect-video bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center">
                    {practice.photos && practice.photos.length > 0 ? (
                      <>
                        ...
                        {practiceStatus && (
                          <Badge
                            className={`absolute top-4 left-4 ${practiceStatus.badgeColor} text-white font-semibold shadow-lg`}
                          >
                            {practiceStatus.isOpen ? (
                              <span className="flex items-center gap-1">
                                <div className="w-2 h-2 bg-white rounded-full animate-pulse"></div>
                                {practiceStatus.statusText}
                              </span>
                            ) : (
                              practiceStatus.statusText
                            )}
                    ...
                    <CardContent className="p-6">
                      ...
                      {practice.photos && practice.photos.length > 0 ? (
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                          {practice.photos.map((photo: string, index: number) => (
            ...
            {/* Booking Sidebar - UPDATED WITH DYNAMIC FUNCTIONALITY */}
            <div className="space-y-6">
              {/* Booking Form */}
              <Card>
                <CardContent className="p-6">
                  ...
                  <div className="mb-6">
                    ...
                    {availableDates.length > 0 ? (
                      <div className="grid grid-cols-5 gap-2">
                        {availableDates
                          .slice(0, 15)
                          .map((dateOption: AvailableDate, index: number) => (
                            <button
                              key={index}
                      ...
                      <div className="text-center py-8 text-gray-500">
                        ...
                        <p className="text-sm">Try navigating to next month</p>
                      </div>
                    )}
                  </div>
                ...
                <CardContent className="p-6">
                  <div className="space-y-3">
                    {/* <Button variant="outline" className="w-full">
                      ...
                      {practice.phoneNumber ? 'Call Now' : 'No Phone Available'}
                    </Button> */}
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600 text-sm flex items-center gap-2">
                        ...
                        {practiceStatus?.isOpen && practiceStatus?.closingTime
                          ? `Open until ${practiceStatus.closingTime}`
                          : practiceStatus?.nextOpening || hours.today}
                      </p>
                    </div>
                  </div>
                </CardContent>
